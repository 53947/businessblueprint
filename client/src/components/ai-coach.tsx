...
export function CoachBlueChat() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ backgroundColor: "#0000FF" }}>
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-3">
        {!activeConversationId && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-8 space-y-3">
            <h3 className="font-semibold text-sm" style={{ color: "#E9ECF0" }}>How can I help your business today?</h3>
            <p className="text-xs max-w-xs" style={{ color: "rgba(233,236,240,0.7)" }}>
            ...
            <div className="flex flex-wrap gap-1.5 justify-center">
              {["How do I get more reviews?", "What should I do first?", "Help with my SEO"].map((s) => (
                <button key={s} onClick={() => setInputMessage(s)} className="px-2.5 py-1 text-xs rounded-full transition-colors" style={{ backgroundColor: "rgba(233,236,240,0.15)", color: "#E9ECF0", border: "1px solid rgba(233,236,240,0.3)" }}>
                  {s}
                </button>
              ))}
            </div>
        ...
        <div className="space-y-3">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className="relative group">
                <div className={`max-w-[85%] px-3 py-2 rounded-2xl text-sm leading-relaxed ${msg.role === "user" ? "rounded-br-md" : "rounded-bl-md"}`} style={msg.role === "user" ? { backgroundColor: "rgba(233,236,240,0.2)", color: "#E9ECF0" } : { backgroundColor: "rgba(233,236,240,0.15)", color: "#E9ECF0" }}>
                  <div className="whitespace-pre-wrap">{msg.content}</div>
                  <div className="text-xs mt-1" style={{ color: "rgba(233,236,240,0.5)" }}>
          ...
          {chatMutation.isPending && (
            <div className="flex justify-start">
              <div className="px-3 py-2 rounded-2xl rounded-bl-md" style={{ backgroundColor: "rgba(233,236,240,0.15)" }}>
                <div className="flex items-center gap-2 text-sm" style={{ color: "rgba(233,236,240,0.7)" }}>
      ...
      {/* Input */}
      <div className="p-3" style={{ borderTop: "1px solid rgba(233,236,240,0.2)", backgroundColor: "#0000FF" }}>
        <div className="flex gap-2 items-end">
          <Textarea
            value={inputMessage}
            onChange={(e) => setInputMessage(e.target.value)}
            ...
            className="resize-none min-h-[40px] max-h-24 text-sm border-0 focus-visible:ring-1"
            style={{ backgroundColor: "rgba(233,236,240,0.1)", color: "#E9ECF0", borderColor: "rgba(233,236,240,0.3)" }}
            rows={1}
            disabled={chatMutation.isPending}
          />
          <Button
