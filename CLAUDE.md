...
| Date | Changes |
|------|---------|
| 2026-04-29 | **Coach Blue semantic-folder cleanup — four CDN refs in bbp now resolve to `coachblue/logo-image.png` instead of `businessblueprint/logo-image.png`.** Closes the out-of-scope item flagged at the close of Prompt 04/27/2026-2 (SHA 92d7720), and additionally fixes a fourth Coach Blue ref that was missed by both the prior prompt and the original draft of this prompt. **Files swapped (4):** `client/src/components/header.tsx` L55 (`coachBlueIcon` const), `client/src/pages/home.tsx` L12 (`coachBlueStepIcon` const — line shifted from L17 to L12 because the 04-27 sweep had inserted `coachBlueLarge` at L10 already pointing at `coachblue/`), `client/src/pages/journey.tsx` L260 (inline JSX `<img>` in the "Never Grow Alone — Coach Blue" step header), `client/src/pages/tour.tsx` L421 (inline JSX `<img>` inside a Coach Blue showcase block — discovered during Gate 4 grep, NOT in the prompt's original 3-file list, scope-expanded with explicit Dean approval before edit). **Untouched (3 legitimate bbp brandmark refs):** `side-nav.tsx` L19 `bbIcon`, `digital-blueprint.tsx` L26 `blueprintIcon`, `brand-logo.tsx` L3 `bbLogoImage` — these are bbp's own brandmark, not Coach Blue, and correctly resolve to `businessblueprint/logo-image.png`. **No visible change** — both URLs serve byte-identical image bytes today. The fix is structural: it decouples Coach Blue references from bbp's brand folder so the two products can evolve independently (e.g. if Coach Blue's face is ever updated without changing bbp's brandmark, code referencing the wrong folder would either update the wrong asset or fail to update the right one). Quote style preserved per file (single quotes in header.tsx; double quotes in home.tsx, journey.tsx, tour.tsx). Alt text, width/height/style attrs, surrounding markup, and variable names untouched. `npx tsc --noEmit` clean. NIXPACKS_INSTALL_CMD=`npm ci --include=dev` verified set on Railway production prior to push (R6 satisfied). |
